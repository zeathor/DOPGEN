classdef CF4 < handle
    ...
    methods
        ...
        function stepChangeLarge(cf4, S)
            %% this function alters the heights and the optima positions
            oldO = cf4.O;
            oldBias = cf4.bias;
            
            theRand = rand(cf4.nFunc,cf4.nDim);
            deltaO = (S).*((cf4.alpha.*sign(theRand)) + (cf4.alphaMax-cf4.alpha).*theRand).*(cf4.domain(2)-cf4.domain(1));
            deltaO = oldO + deltaO;
            ...
            deltaB = oldBias+S.*((cf4.alpha.*sign(theRand)) + (cf4.alphaMax-cf4.alpha).*theRand).*(cf4.pHeight(2)-cf4.pHeight(1));
        ...
        function stepChangePeakFunction(cf4,severity)         
            prob = rand*2-1;
            if abs(prob) < severity/100
                cf4.nFunc = cf4.nFunc + sign(randi(2)*2-3);
            end
            
            if cf4.nFunc < cf4.funcRange(1)
                cf4.nFunc = cf4.funcRange(1)+1;
            elseif cf4.nFunc > cf4.funcRange(2)
                cf4.nFunc = cf4.funcRange(1)-1;
            end
        end
        ...
        function stepChangeGradual(cf4,severity)
            ...
            deltaB = oldBias + s*(rand(1,cf4.nFunc)*2-1)*(cf4.pHeight(2)-cf4.pHeight(1));
            deltaB(deltaB > (cf4.pHeight(2))) = 2*(cf4.pHeight(2)) - deltaB(deltaB > (cf4.pHeight(2)));
            deltaB(deltaB < cf4.pHeight(1)) = 2*cf4.pHeight(1) - deltaB(deltaB < cf4.pHeight(1));
        ...
        function stepChangeRandom(cf4,severity)
            s = severity;
            
            %% do centres first
            deltaO = s*randn(cf4.nFunc, cf4.nDim)*(cf4.domain(2)-cf4.domain(1))/100+cf4.O(1:cf4.nFunc,:);
            ...
            oldBias = cf4.bias;
            %% do heights
            deltaB = s*randn(1,cf4.nFunc)*(cf4.pHeight(2)-cf4.pHeight(1))/100+oldBias;
            deltaB(deltaB > cf4.pHeight(2)) = 2*(cf4.pHeight(2)) - deltaB(deltaB > cf4.pHeight(2));
            deltaB(deltaB < cf4.pHeight(1)) = 2*(cf4.pHeight(1)) - deltaB(deltaB < cf4.pHeight(1));
