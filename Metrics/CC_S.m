function [MLM] = CC_S(DOP, pos, dynSettings, dim, dynamic,DOPLength,DOPStrm)
    ...
    for iter = 1:DOPLength
        ...
        if(isequal(dynamic, @TCD) || isequal(dynamic,@RCD) || isequal(dynamic,@TRCD_S) || isequal(dynamic,@RCDF_S) || isequal(dynamic,@RCDFix))
            [posC, DOPidx] = dynamic(DOP, posC, dynSettings, dim,iter, DOPidx,DOPStrm);
        else    
